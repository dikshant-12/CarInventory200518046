...
import com.example.carinventory.databinding.FragmentSoldCarsBinding;
...
public class SoldCars extends Fragment {


    FragmentSoldCarsBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentSoldCarsBinding.inflate(inflater,container,false);
    ...
    private void refreshData(){
        arrayList.clear();
        try {
            ArrayList<CarModel> carsArrayList = Paper.book().read("carsArrayList", new ArrayList<CarModel>());
            if (carsArrayList != null) {
                for (int i=0;i<carsArrayList.size();i++){
                    CarModel model = carsArrayList.get(i);
                    if (model.is_sold){
                        arrayList.add(model);
                    }
                }
